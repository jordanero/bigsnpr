...
snp_ssctpr <- function(corr, df_beta,
                       ...
                       nlambda1 = 20, lambda1.min.ratio = 0.01,
                       nlambda2 = 20, lambda2.min.ratio = 0.01, lambda2_max = 1e-3,
		       dfmax = 200e3, maxiter = 500, 
		       tol = 1e-5, ncores = 1) {
    
  lambda1_start <- max(abs(beta_hat))
  seq_lambda1 <- seq_log(lambda1_start, lambda1.min.ratio * lambda1_start, nlambda1 + 1)[-1]
  seq_lambda2 <- seq_log(lambda2_max, lambda2.min.ratio * lambda2_max, nlambda2 + 1)[-1]
